...
export async function ensureOrderDraftTables() {
  ...
  await sql`CREATE TABLE IF NOT EXISTS sales_order_drafts (id SERIAL PRIMARY KEY, draft_number VARCHAR(40) UNIQUE NOT NULL, account_id INTEGER NOT NULL, customer_name VARCHAR(255) NOT NULL, order_date DATE NOT NULL DEFAULT CURRENT_DATE, requested_delivery_date DATE NOT NULL, deposit_amount NUMERIC(15,2) NOT NULL DEFAULT 0, notes TEXT, delivery_address TEXT, contact_phone VARCHAR(50), priority VARCHAR(20) NOT NULL DEFAULT 'normal', checklist_template_id INTEGER REFERENCES order_checklist_templates(id), attachments JSONB NOT NULL DEFAULT '[]'::jsonb, status VARCHAR(20) NOT NULL DEFAULT 'draft', created_by VARCHAR(100), branch_id INTEGER, confirmed_order_id INTEGER, checklist_values JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)`
  await sql`ALTER TABLE sales_order_drafts ADD COLUMN IF NOT EXISTS account_id INTEGER`
  await sql`UPDATE sales_order_drafts d SET account_id = COALESCE(c.account_id, d.customer_id) FROM customers c WHERE d.account_id IS NULL AND c.id = d.customer_id`
