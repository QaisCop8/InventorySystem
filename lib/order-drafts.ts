...
export async function ensureOrderDraftTables() {
  ...
  await sql`CREATE TABLE IF NOT EXISTS users_currencies_default_account_tbl (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES user_settings(user_id), currency_id INTEGER, account_id INTEGER, received_cheqs_account_id INTEGER, returned_cheqs_account_id INTEGER, cards_account_id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`
  ...
  await sql`CREATE TABLE IF NOT EXISTS sales_order_drafts (id SERIAL PRIMARY KEY, draft_number VARCHAR(40) UNIQUE NOT NULL, account_id INTEGER NOT NULL, customer_id INTEGER, customer_name VARCHAR(255) NOT NULL, order_date DATE NOT NULL DEFAULT CURRENT_DATE, requested_delivery_date DATE NOT NULL, deposit_amount NUMERIC(15,2) NOT NULL DEFAULT 0, notes TEXT, delivery_address TEXT, contact_phone VARCHAR(50), priority VARCHAR(20) NOT NULL DEFAULT 'normal', checklist_template_id INTEGER REFERENCES order_checklist_templates(id), attachments JSONB NOT NULL DEFAULT '[]'::jsonb, status VARCHAR(20) NOT NULL DEFAULT 'draft', created_by INTEGER REFERENCES user_settings(user_id), branch_id INTEGER, confirmed_order_id INTEGER, checklist_values JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)`
  ...
  await sql`CREATE TABLE IF NOT EXISTS sales_order_draft_events (id SERIAL PRIMARY KEY, draft_id INTEGER NOT NULL REFERENCES sales_order_drafts(id) ON DELETE CASCADE, event_type VARCHAR(50) NOT NULL, user_id INTEGER REFERENCES user_settings(user_id), details JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)`
