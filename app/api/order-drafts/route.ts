...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!d.customer_id || !d.requested_delivery_date || !Array.isArray(d.items) || !d.items.length) return NextResponse.json({ error: "العميل وتاريخ التسليم وصنف واحد على الأقل مطلوبة" }, { status: 400 })
    ...
    const [draft] = await sql`INSERT INTO sales_order_drafts (draft_number,customer_id,customer_name,order_date,requested_delivery_date,deposit_amount,notes,delivery_address,contact_phone,priority,checklist_template_id,attachments,created_by,branch_id) VALUES (${number},${d.customer_id},${d.customer_name},${d.order_date || new Date().toISOString().slice(0,10)},${d.requested_delivery_date},${Number(d.deposit_amount)||0},${d.notes||null},${d.delivery_address||null},${d.contact_phone||null},${d.priority||"normal"},${d.checklist_template_id||null},${JSON.stringify(attachments)}::jsonb,${d.created_by||null},${d.branch_id||null}) RETURNING *`
