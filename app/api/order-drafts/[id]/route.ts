...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const permission = await requirePermissionByName(_request, "استعلام مسودة طلبية مبيعات")
    ...
    const rows: any[] = await sql`
      SELECT d.*, COALESCE(json_agg(i ORDER BY i.id) FILTER (WHERE i.id IS NOT NULL), '[]') AS items
      FROM sales_order_drafts d
      LEFT JOIN sales_order_draft_items i ON i.draft_id = d.id
      WHERE d.id = ${id}
      GROUP BY d.id
    `
    if (!rows.length) return NextResponse.json({ error: "المسودة غير موجودة" }, { status: 404 })
    const { customer_id: _deprecatedCustomerId, ...draft } = rows[0]
    return NextResponse.json(draft)
  } catch (error: any) {
    return NextResponse.json({ error: error.message || "تعذر تحميل المسودة" }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const permission = await requirePermissionByName(request, "تعديل مسودة طلبية مبيعات", Number(data.branch_id))
    if (!permission.ok) return permission.response
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const permission = await requirePermissionByName(_request, "تعديل مسودة طلبية مبيعات")
    if (!permission.ok) return permission.response
    await ensureOrderDraftTables()
    const id = Number((await params).id)
