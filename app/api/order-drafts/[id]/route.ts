...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const item of data.items) await client.query(`INSERT INTO sales_order_draft_items (draft_id,product_id,product_name,quantity,price,discount,unit_id,barcode,store_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [id, Number(item.product_id), item.product_name, Number(item.quantity), Number(item.price), Number(item.discount), item.unit_id || null, item.barcode || null, item.store_id || null])
