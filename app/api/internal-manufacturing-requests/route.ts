...
export async function GET(request: NextRequest) {
  try { await ensureInternalManufacturingTables(); const user = await getSessionUser(request); if (!user) return NextResponse.json({ error: "يجب تسجيل الدخول" }, { status: 401 }); const branchId = Number(request.headers.get("x-branch-id")); await authorizeInternalManufacturing(user.user_id, branchId, "create"); const status = Number(new URL(request.url).searchParams.get("status") || 0); return NextResponse.json(await listInternalManufacturingRequests(status || undefined)) } catch (error: any) { return NextResponse.json({ error: error.message }, { status: 400 }) }
}
