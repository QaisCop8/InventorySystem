...
export async function GET(request:NextRequest){
  try { await ensureCustomerProductTemplateTables(); const code=request.nextUrl.searchParams.get("code")?.trim(); const nav=request.nextUrl.searchParams.get("navigation"); const current=Number(request.nextUrl.searchParams.get("currentId")||0)
    if(request.nextUrl.searchParams.get("generate")==="1"){const rows:any[]=await sql`SELECT COALESCE(MAX(id),0)+1 next FROM customer_product_templates_tbl`;return NextResponse.json({code:`CPT-${String(rows[0].next).padStart(5,"0")}`})}
...
export async function POST(request:NextRequest){let client:any;try{await ensureCustomerProductTemplateTables();const d=await request.json(),error=validate(d);if(error)return NextResponse.json({error},{status:400});client=await(await getTenantPool()).connect();await client.query("BEGIN");const duplicate=await client.query("SELECT id FROM customer_product_templates_tbl WHERE status<>3 AND (UPPER(template_code)=UPPER($1) OR LOWER(TRIM(name_ar))=LOWER(TRIM($2)))",[String(d.template_code).trim(),String(d.name_ar).trim()]);if(duplicate.rowCount)throw new Error("كود أو اسم القالب مستخدم مسبقاً");const customers=await client.query("SELECT id FROM account_tbl WHERE id=ANY($1::int[]) AND type=2 AND COALESCE(status,1) IN (1,2)",[d.customer_ids.map(Number)]);if(customers.rowCount!==new Set(d.customer_ids.map(Number)).size)throw new Error("يوجد حساب عميل غير صالح");const products=await client.query("SELECT id FROM products WHERE id=ANY($1::int[]) AND COALESCE(status,1)<>3",[d.product_ids.map(Number)]);if(products.rowCount!==new Set(d.product_ids.map(Number)).size)throw new Error("يوجد صنف غير صالح");const saved=await client.query("INSERT INTO customer_product_templates_tbl(template_code,name_ar,name_en,start_date,end_date,status,notes) VALUES($1,$2,$3,$4,$5,1,$6) RETURNING *",[String(d.template_code).trim().toUpperCase(),String(d.name_ar).trim(),String(d.name_en||"").trim()||null,d.start_date,d.end_date||null,d.notes||null]);for(const id of new Set(d.customer_ids.map(Number)))await client.query("INSERT INTO customer_product_template_customers_tbl(template_id,account_id) VALUES($1,$2)",[saved.rows[0].id,id]);for(const id of new Set(d.product_ids.map(Number)))await client.query("INSERT INTO customer_product_template_products_tbl(template_id,product_id) VALUES($1,$2)",[saved.rows[0].id,id]);await client.query("COMMIT");return NextResponse.json(saved.rows[0],{status:201})}catch(e:any){if(client)await client.query("ROLLBACK").catch(()=>{});return NextResponse.json({error:e.message},{status:e.message?.includes("مسبقاً")||e.message?.includes("غير صالح")?400:500})}finally{client?.release()}}
