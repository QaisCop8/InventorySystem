  ...
  try { await ensureCustomerProductTemplateTables(); const code=request.nextUrl.searchParams.get("code")?.trim(); const nav=request.nextUrl.searchParams.get("navigation"); const current=Number(request.nextUrl.searchParams.get("currentId")||0)
    ...
    if(request.nextUrl.searchParams.get("lookup")==="1"){
      const [templates,customers,products]=await Promise.all([
        ...
        sql`SELECT id,id AS account_id,code AS account_code,name AS account_name FROM account_tbl WHERE type=2 AND COALESCE(status::text,'1') IN ('1','2','active','ACTIVE','نشط') ORDER BY code`,
        sql`SELECT id,product_code,product_name,barcode FROM products WHERE COALESCE(status::text,'1') NOT IN ('3','deleted','DELETED') AND COALESCE(type::text,'1')='1' ORDER BY product_code`,
      ])
