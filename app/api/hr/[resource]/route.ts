...
export async function GET(request: NextRequest, { params }: { params: { resource: string } }) {
  ...
  try {
    ...
    if (resource === "lookups") {
      const [jobs, departments, branches, employees, items, exemptions, taxLaws, currencies, voucherTypes] = await Promise.all([sql`SELECT * FROM employee_jobs_tbl WHERE is_active ORDER BY id`, sql`SELECT * FROM departments WHERE is_active ORDER BY id`, sql`SELECT * FROM branches WHERE is_active ORDER BY id`, sql`SELECT id,employee_code,full_name,department_id,branch_id FROM employees_tbl WHERE COALESCE(status,1)=1 ORDER BY employee_code`, sql`SELECT * FROM salary_items_tbl WHERE is_active ORDER BY id`, sql`SELECT * FROM tax_exemptions_tbl WHERE is_active ORDER BY id`, sql`SELECT * FROM tax_laws_tbl WHERE is_active ORDER BY id`, sql`SELECT id, currency_code, currency_name FROM currency WHERE COALESCE(is_active,true) ORDER BY id`, sql`SELECT id,name FROM voucher_types_tbl WHERE COALESCE(status,1)<>3 ORDER BY id`])
      return NextResponse.json({ jobs, departments, branches, employees, items, exemptions, taxLaws, currencies, voucherTypes })
    }
    ...
    if (resource === "attendance-reports") {
      const from = request.nextUrl.searchParams.get("from") || "1900-01-01"
      const to = request.nextUrl.searchParams.get("to") || "2999-12-31"
      const employeeIds = (request.nextUrl.searchParams.get("employee_ids") || "").split(",").map(Number).filter(Boolean)
      const departmentId = Number(request.nextUrl.searchParams.get("department_id") || 0)
      const branchId = Number(request.nextUrl.searchParams.get("branch_id") || 0)
        const punches = await sql`
          SELECT e.id employee_id,e.employee_code,e.full_name,e.department_id,e.branch_id,d.department_name,b.branch_name,
            l.punch_time,l.punch_type,l.verification_type,l.device_user_id,
            r.shift_id,s.name shift_name,s.start_time,s.end_time,s.break_minutes
          FROM attendance_logs_tbl l
          JOIN employees_tbl e ON e.id=l.employee_id OR e.employee_code=l.employee_code
          LEFT JOIN departments d ON d.id=e.department_id
          LEFT JOIN branches b ON b.id=e.branch_id
          LEFT JOIN shift_schedule_rules_tbl r ON r.employee_id=e.id AND r.date_from=l.punch_time::date
          LEFT JOIN shift_definitions_tbl s ON s.id=r.shift_id
          WHERE l.punch_time >= ${from}::date AND l.punch_time < (${to}::date + INTERVAL '1 day')
            AND (${employeeIds.length === 0} OR e.id=ANY(${employeeIds}::int[]))
            AND (${departmentId}=0 OR e.department_id=${departmentId})
            AND (${branchId}=0 OR e.branch_id=${branchId})
          ORDER BY e.employee_code,l.punch_time
        `
        const isExit = (type: any, hasOpen: boolean) => { const value = String(type || "").trim(); if (/^(out|exit|خروج|2|3)$/i.test(value)) return true; if (/^(in|entry|دخول|1)$/i.test(value)) return false; return hasOpen }
        const requiredMinutes = (row: any) => {
          if (!row.start_time || !row.end_time) return 0
          const start = String(row.start_time).split(":").map(Number); const end = String(row.end_time).split(":").map(Number)
          let value = (end[0] * 60 + end[1]) - (start[0] * 60 + start[1]); if (value < 0) value += 24 * 60
          return Math.max(0, value - (Number(row.break_minutes) || 0))
        }
        const employeeDays: Record<string, any> = {}
        const open: Record<string, any> = {}
        for (const punch of punches as any[]) {
          const employeeKey = String(punch.employee_id); const date = new Date(punch.punch_time).toISOString().slice(0, 10); const key = `${employeeKey}:${date}`
          const day = employeeDays[key] ||= { employee_id: punch.employee_id, employee_code: punch.employee_code, full_name: punch.full_name, department_name: punch.department_name, branch_name: punch.branch_name, work_date: date, sessions: [] }
          const exit = isExit(punch.punch_type, Boolean(open[employeeKey]))
          if (!open[employeeKey] && !exit) { open[employeeKey] = { entry: punch, day }; continue }
          if (open[employeeKey] && exit) {
            const pair = open[employeeKey]; const worked = Math.max(0, Math.round((new Date(punch.punch_time).getTime() - new Date(pair.entry.punch_time).getTime()) / 60000)); const required = requiredMinutes(pair.entry)
            pair.day.sessions.push({ entry_time: pair.entry.punch_time, entry_type: pair.entry.punch_type || "in", exit_time: punch.punch_time, exit_type: punch.punch_type || "out", worked_minutes: worked, required_minutes: required, difference_minutes: worked - required, shift_name: pair.entry.shift_name || "" }); delete open[employeeKey]; continue
          }
          if (!open[employeeKey] && exit) continue
        }
        for (const item of Object.values(open)) { const required = requiredMinutes(item.entry); item.day.sessions.push({ entry_time: item.entry.punch_time, entry_type: item.entry.punch_type || "in", exit_time: null, exit_type: "", worked_minutes: 0, required_minutes: required, difference_minutes: -required, shift_name: item.entry.shift_name || "" }) }
        const detail = Object.values(employeeDays)
        const summary = Object.values(detail.reduce((map: Record<string, any>, row: any) => { const key=String(row.employee_id); const current=map[key] || { employee_id: row.employee_id, employee_code: row.employee_code, employee_name: row.full_name, department_name: row.department_name, branch_name: row.branch_name, days: 0, worked_minutes: 0, required_minutes: 0, difference_minutes: 0, late_days: 0 }; current.days++; row.sessions.forEach((session: any) => { current.worked_minutes += session.worked_minutes; current.required_minutes += session.required_minutes; current.difference_minutes += session.difference_minutes }); if (current.difference_minutes < 0) current.late_days++; map[key]=current; return map }, {}))
        return NextResponse.json({ detail, summary })
    }
