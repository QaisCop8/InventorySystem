...
export async function POST(request:NextRequest){
 try{
  ...
  return await withTenantTransaction(async()=>{
   ...
   const branchIds=Array.from(new Set(cheques.map((x:any)=>Number(x.source_branch_id)).filter(Boolean))),currencyIds=Array.from(new Set(cheques.map((x:any)=>Number(x.currency_id)||0)))
   if(branchIds.length!==1)return NextResponse.json({error:"يجب أن تكون جميع الشيكات من نفس الفرع"},{status:400})
   if(currencyIds.length!==1)return NextResponse.json({error:"يجب أن تكون جميع الشيكات من نفس العملة"},{status:400})
   const branchId=branchIds[0],createAuth=await authorizeTransaction(request,"journal","create",branchId);if(!createAuth.ok)return createAuth.response
   const postAuth=await authorizeTransaction(request,"journal","post",branchId);if(!postAuth.ok)return postAuth.response
   for(const cheque of cheques){const businessDate=String(cheque.business_date);if(Number(cheque.cheq_type)!==config.type)return NextResponse.json({error:`نوع الشيك ${cheque.cheq_num} لا يناسب العملية`},{status:409});if(operation.requiresDue&&String(cheque.due_date||"").slice(0,10)>businessDate)return NextResponse.json({error:`الشيك ${cheque.cheq_num} غير مستحق`},{status:409});if(!isChequeOperationAllowed(operation,cheque,businessDate))return NextResponse.json({error:`الحالة الحالية للشيك ${cheque.cheq_num} لا تسمح بالعملية`},{status:409})}
   let targetAccountId:number|null=null,targetBankAccountId:number|null=null
   if(code==="deposit"){
    if(!requestedAccountId)return NextResponse.json({error:"اختر حساب البنك"},{status:400});const bank=(await sql`SELECT id,jary_account_id FROM bank_accounts WHERE id=${requestedAccountId} AND COALESCE(status,1)<>3`)[0];if(!bank?.jary_account_id)return NextResponse.json({error:"الحساب الجاري غير معرّف في حساب البنك"},{status:400});targetAccountId=Number(bank.jary_account_id);targetBankAccountId=Number(bank.id)
   }else if(code==="endorse"){
    if(!requestedAccountId)return NextResponse.json({error:"اختر حساب المستفيد من التجيير"},{status:400});const account=(await sql`SELECT id FROM account_tbl WHERE id=${requestedAccountId} AND COALESCE(status,1)<>3`)[0];if(!account)return NextResponse.json({error:"حساب المستفيد غير موجود أو غير فعال"},{status:400});targetAccountId=Number(account.id)
   }
   const lines:any[]=[]
   for(const cheque of cheques){const current=Number(cheque.current_account_id||cheque.rec_cheq_account_id),rate=Number(cheque.rate||1),amount=Number(cheque.amount||0),currencyId=Number(cheque.currency_id)||null;if(!current||amount<=0)return NextResponse.json({error:`تعذر تحديد حساب أو قيمة الشيك ${cheque.cheq_num}`},{status:400})
    if(code==="deposit"||code==="endorse"){lines.push({accountId:Number(targetAccountId),creditDebit:1,amount,currencyId,rate,note:`${config.name} - شيك ${cheque.cheq_num}`},{accountId:current,creditDebit:2,amount,currencyId,rate,note:`${config.name} - شيك ${cheque.cheq_num}`})}
    else{const bankId=Number(cheque.bank_account_id);if(!bankId)return NextResponse.json({error:`لا يوجد حساب بنك مرتبط بالشيك ${cheque.cheq_num}`},{status:400});const bank=(await sql`SELECT jary_account_id FROM bank_accounts WHERE id=${bankId} AND COALESCE(status,1)<>3`)[0];if(!bank?.jary_account_id)return NextResponse.json({error:`الحساب الجاري غير معرّف للشيك ${cheque.cheq_num}`},{status:400});cheque.target_account_id=Number(bank.jary_account_id);lines.push({accountId:current,creditDebit:1,amount,currencyId,rate,note:`${config.name} - شيك ${cheque.cheq_num}`},{accountId:Number(bank.jary_account_id),creditDebit:2,amount,currencyId,rate,note:`${config.name} - شيك ${cheque.cheq_num}`})}
   }
   const journal=await createChequeBulkOperationJournal({userId:String(user.user_id),branchId,operationName:config.name,operationDate,currencyId:currencyIds[0]||null,rate:Number(cheques[0].rate||1),note:note||`${config.name} (${ids.length} شيكات)`,lines})
   if(!journal.ok)return NextResponse.json({error:journal.error},{status:400})
   for(const cheque of cheques){const newCurrent=code==="clear_outgoing"?Number(cheque.target_account_id):Number(targetAccountId),newStatus=code==="endorse"?7:4,accountId=newCurrent,bankId=code==="deposit"?targetBankAccountId:(Number(cheque.bank_account_id)||null)
   ...
   return NextResponse.json({message:`تم تنفيذ ${config.name} لعدد ${ids.length} شيكات وإنشاء سند القيد ${journal.code}`,count:ids.length,journal_voucher:journal})
  })
