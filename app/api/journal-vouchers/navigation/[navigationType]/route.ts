...
export async function GET(request: NextRequest, { params }: { params: Promise<{ navigationType: string }> }) {
  try {
    ...
    switch (navigationType) {
      case "first":
        rows = await sql`SELECT * FROM voucher_header_tbl WHERE vch_type = ${JOURNAL_VCH_TYPE} AND status != 3 AND branch_id = ANY(${authorization.branchIds}::int[]) ORDER BY id ASC LIMIT 1`
        break
      case "last":
        rows = await sql`SELECT * FROM voucher_header_tbl WHERE vch_type = ${JOURNAL_VCH_TYPE} AND status != 3 AND branch_id = ANY(${authorization.branchIds}::int[]) ORDER BY id DESC LIMIT 1`
        break
      case "previous":
        rows = await sql`SELECT * FROM voucher_header_tbl WHERE id < ${currentId || 0} AND vch_type = ${JOURNAL_VCH_TYPE} AND status != 3 AND branch_id = ANY(${authorization.branchIds}::int[]) ORDER BY id DESC LIMIT 1`
        break
      case "next":
        rows = await sql`SELECT * FROM voucher_header_tbl WHERE id > ${currentId || 0} AND vch_type = ${JOURNAL_VCH_TYPE} AND status != 3 AND branch_id = ANY(${authorization.branchIds}::int[]) ORDER BY id ASC LIMIT 1`
        break
      default:
