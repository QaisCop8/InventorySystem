...
export function DraftOrdersPage() {
  ...
  const [selectedStatuses, setSelectedStatuses] = useState<string[]>(["draft", "ready"])
  ...
  const visible = useMemo(() => {
    ...
    return drafts.filter((draft) => {
      const filterStatus = draft.status === "confirmed" && draft.confirmed_order_id ? "ready" : "draft"
      return selectedStatuses.includes(filterStatus) && (!term || String(draft.draft_number || "").toLowerCase().includes(term) || String(draft.customer_name || "").toLowerCase().includes(term))
    })
  }, [drafts, search, selectedStatuses])

  const statusLabel = (draft: any) => draft.status === "confirmed" && draft.confirmed_order_id ? "جاهز" : draft.status === "cancelled" ? "ملغاة" : "مسودة"
  ...
  return <div dir="rtl" className="space-y-5 p-3 md:p-6">
    ...
    <Card><CardContent className="grid gap-3 pt-6 md:grid-cols-[minmax(0,1fr)_minmax(240px,360px)_auto] md:items-end">
      ...
      <div className="min-w-0"><label className="mb-2 block text-sm font-medium">حالة المسودة</label><MultiSelect value={selectedStatuses} options={statusOptions} optionLabel="label" optionValue="value" showCheck showMultiSelect placeholder="اختر حالة المسودة" selectedItemsLabel="تم تحديد {0} حالات" selectAllLabel="تحديد الكل" onChange={(event: any) => setSelectedStatuses(Array.isArray(event.value) ? event.value.map(String) : [])} onSelectAll={(event: any) => setSelectedStatuses(Array.isArray(event?.value) ? event.value.map((value: any) => String(value?.value ?? value)) : event?.checked ? statusOptions.map((option) => option.value) : [])} /></div>
    ...
    {loading ? <div className="py-16 text-center text-muted-foreground">جاري تحميل المسودات...</div> : visible.length === 0 ? <div className="rounded-xl border-2 border-dashed py-16 text-center"><FileText className="mx-auto mb-3 h-10 w-10 text-muted-foreground" /><h2 className="font-bold">لا توجد مسودات</h2><p className="mb-4 mt-1 text-sm text-muted-foreground">ابدأ بإنشاء أول مسودة طلبية.</p><Button onClick={() => setOpen(true)}><Plus className="ml-2 h-4 w-4" />إنشاء مسودة طلبية</Button></div> :
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((draft) => <Card key={draft.id} className="transition hover:border-emerald-300 hover:shadow-md"><CardContent className="space-y-4 pt-6">
        <div className="flex items-start justify-between gap-3"><div><div className="font-bold">{draft.draft_number}</div><div className="mt-1 text-sm text-muted-foreground">{draft.customer_name}</div></div><Badge variant={draft.status === "confirmed" && draft.confirmed_order_id ? "default" : "secondary"}>{statusLabel(draft)}</Badge></div>
    ...
    <Dialog modal={false} open={open} onOpenChange={setOpen}><DialogContent className="min-w-0 max-h-[94vh] w-[calc(100%-1rem)] max-w-[1400px] overflow-x-hidden overflow-y-auto rounded-xl border-4 border-emerald-600 p-0 shadow-[0_24px_80px_rgba(0,0,0,0.35)] ring-4 ring-emerald-600/20" dir="rtl" onPointerDownOutside={(event) => event.preventDefault()} onInteractOutside={(event) => event.preventDefault()}>
