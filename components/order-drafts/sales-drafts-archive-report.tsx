...
export default function SalesDraftsArchiveReport() {
  ...
  return <ReportPage>
    <ReportHeader title={<>تقرير ارشفة مسودات طلبيات المبيعات</>} description={<>تتبع المسودة منذ إنشائها وحتى فحص الإنتاج وتحويلها إلى طلبية مبيعات.</>} actions={<Button onClick={() => void load()} disabled={loading}><RefreshCw className={`ml-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />تحديث التقرير</Button>} />
    ...
    <ReportFilters><CardContent className="grid gap-3 p-4 md:grid-cols-[1fr_170px_170px_180px_auto] md:items-end"><div><Label>بحث</Label><div className="relative mt-1"><Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input data-report-apply-on-enter className="pr-9" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void load()} placeholder="رقم المسودة أو العميل أو الطلبية" /></div></div><div><Label>من تاريخ</Label><Input className="mt-1" type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></div><div><Label>إلى تاريخ</Label><Input className="mt-1" type="date" value={to} onChange={(event) => setTo(event.target.value)} /></div><div><Label>الحالة</Label><select className="mt-1 h-10 w-full rounded-md border bg-background px-3" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">الكل</option><option value="draft">مسودة</option><option value="confirmed">تحولت إلى طلبية</option><option value="cancelled">ملغاة</option></select></div><Button data-report-apply onClick={() => void load()}>تطبيق</Button></CardContent></ReportFilters>
