...
export function AttendanceSummaryReportPage() {
  ...
  return <HrPage report title="دخول الموظفين إجمالي" subtitle="سجل الحضور اليومي للموظفين"><ReportFiltersBar value={filters} lookups={lookups} onChange={setFilters} onLoad={() => void load()} /><Grid rows={rows} columns={dailyColumns} height="65vh" />{loading && <p className="text-sm text-slate-500">جاري تحميل التقرير...</p>}</HrPage>
}
