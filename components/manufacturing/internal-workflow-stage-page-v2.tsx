...
export default function InternalWorkflowStagePageV2({ stage }: { stage: Stage }) {
  ...
  const load = async () => {
    ...
    if (!branchId) return
    const response = await fetch(`/api/internal-manufacturing-requests?status=${stage.status}&_=${Date.now()}`, { cache: "no-store", headers: { "x-branch-id": String(branchId) } })
    const data = await response.json()
    if (sequence !== loadSequenceRef.current) return
    if (response.ok && Array.isArray(data)) setRequests(data)
    else setMessage(data.error || "تعذر تحميل الطلبات")
  }
  useEffect(() => { if (activeBranchId) void load(); else { loadSequenceRef.current += 1; setRequests([]) }; Promise.all([fetch("/api/branches"), fetch("/api/warehouses")]).then(async ([branchesResponse, warehousesResponse]) => { setBranches(await branchesResponse.json()); setWarehouses(await warehousesResponse.json()) }) }, [activeBranchId, stage.status])
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      itemElements.forEach((element, index) => {
        ...
        if (!item || element.querySelector("[data-internal-item-image]")) return
        element.classList.add("flex", "items-center", "gap-3", "font-bold", "text-blue-600")
        const imageContainer = document.createElement(item.product_image ? "img" : "div")
        imageContainer.dataset.internalItemImage = "true"
        imageContainer.className = "flex h-16 w-16 shrink-0 items-center justify-center rounded border bg-muted/30 object-cover text-[10px] text-muted-foreground"
        if (imageContainer instanceof HTMLImageElement) { imageContainer.src = item.product_image; imageContainer.alt = item.item_name || "" }
        else imageContainer.textContent = "لا صورة"
        const unit = document.createElement("span")
        unit.dataset.internalItemUnit = "true"
        unit.className = "mr-auto text-sm font-normal text-red-600"
