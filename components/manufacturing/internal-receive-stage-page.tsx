...
export default function InternalReceiveStagePage({ stage }: { stage: Stage }) {
  ...
  const load = async () => { const branchId = Number(activeBranchId || 0); const sequence = ++loadSequenceRef.current; if (!branchId) return; const response = await fetch(`/api/internal-manufacturing-requests?status=${stage.status}&_=${Date.now()}`, { cache: "no-store", headers: { "x-branch-id": String(branchId) } }); const data = await response.json(); if (sequence !== loadSequenceRef.current) return; if (response.ok && Array.isArray(data)) setRequests(data); else setMessage(data.error || "تعذر تحميل الطلبات") }
  useEffect(() => { if (activeBranchId) void load(); else { loadSequenceRef.current += 1; setRequests([]) } }, [activeBranchId, stage.status])
  useEffect(() => {
    if (!selected) return
  ...
  useEffect(() => {
    ...
    let frame = 0
    if (selected) frame = window.requestAnimationFrame(() => {
      const dialog = document.querySelector('[role="dialog"][data-state="open"]')
