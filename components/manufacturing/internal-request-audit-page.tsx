...
export default function InternalRequestAuditPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/internal-manufacturing-requests?status=2", { headers: { "x-branch-id": String(activeBranchId || "") } })
  ...
  return <div dir="rtl" className="space-y-5 p-3 md:p-6"><div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">تدقيق طلب البضاعة</h1><p className="mt-1 text-sm text-muted-foreground">راجع بيانات الطلب وعدّل الأصناف قبل الاعتماد.</p></div><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="ml-2 h-4 w-4" />تحديث</Button></div>{message && <div className="rounded border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">{message}</div>}<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{requests.map((request) => <Card key={request.id} className="cursor-pointer" onClick={() => openRequest(request)}><CardHeader className="pb-3"><CardTitle className="flex items-center justify-between text-base"><span>{request.vch_code}</span><Badge>قيد التدقيق</Badge></CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><div>التاريخ: <b>{String(request.vch_date).slice(0, 10)}</b></div><div>مقدم الطلب: <b>{request.requester_name || "-"}</b></div><div>الأصناف: <b>{request.items?.length || 0}</b></div><Button className="w-full" variant="outline" onClick={(event) => { event.stopPropagation(); openRequest(request) }}><Edit className="ml-2 h-4 w-4" />فتح الطلب</Button></CardContent></Card>)}</div>{!loading && requests.length === 0 && <div className="rounded-xl border-2 border-dashed py-16 text-center"><FileText className="mx-auto mb-3 h-10 w-10 text-emerald-600" />لا توجد طلبات قيد التدقيق</div>}<Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}><DialogContent dir="rtl" className="max-h-[94vh] max-w-4xl overflow-y-auto" onPointerDownOutside={(event) => event.preventDefault()} onInteractOutside={(event) => event.preventDefault()}><DialogHeader><DialogTitle>تدقيق طلب {selected?.vch_code}</DialogTitle></DialogHeader>{selected && <div className="space-y-4"><div className="grid gap-3 rounded border p-4 sm:grid-cols-2"><div>فرع مقدم الطلب: <b>{branchName(selected.branch_id)}</b></div><div>فرع البضاعة: <b>{branchName(selected.manufacturing_branch_id)}</b></div><div>مستودع مقدم الطلب: <b>{warehouseName(selected.to_store_id)}</b></div><div>مستودع المطلوب من البضاعة: <b>{warehouseName(selected.destination_warehouse_id)}</b></div><div>مقدم الطلب: <b>{selected.requester_name || "-"}</b></div></div><div className="rounded border p-4"><h3 className="mb-3 font-bold">الأصناف</h3>{items.map((item, index) => <div key={`${item.item_id}-${index}`} className="mb-2 grid grid-cols-[1fr_120px_40px] items-center gap-2"><Input value={item.item_name || ""} disabled /><Input type="number" min="1" value={item.quantity} onChange={(event) => setItems((current) => current.map((entry, itemIndex) => itemIndex === index ? { ...entry, quantity: Number(event.target.value) } : entry))} /><Button type="button" variant="ghost" size="icon" onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="h-4 w-4" /></Button></div>)}</div><div className="flex gap-2"><Button className="flex-1" onClick={() => void saveItems()} disabled={saving}><Save className="ml-2 h-4 w-4" />حفظ التعديلات</Button><Button className="flex-1" variant="default" onClick={() => void complete()} disabled={saving}><CheckCircle2 className="ml-2 h-4 w-4" />اعتماد الطلب</Button></div></div>}</DialogContent></Dialog></div>
}
