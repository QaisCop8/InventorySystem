...
export function PosPaymentDetailsDialog(props:Props){
 ...
 const customerField=field("رقم العميل",<Select value={customerId?String(customerId):"none"} onValueChange={value=>onCustomerChange(value==="none"?null:Number(value))}><SelectTrigger><SelectValue placeholder="اختر العميل"/></SelectTrigger><SelectContent><SelectItem value="none">اختر العميل</SelectItem>{customers.map(row=><SelectItem key={row.id} value={String(row.id)}>{row.code||row.id} — {row.name}</SelectItem>)}</SelectContent></Select>,tab==="cheque"||tab==="account")
