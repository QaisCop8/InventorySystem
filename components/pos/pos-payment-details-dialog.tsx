...
export function PosPaymentDetailsDialog(props:Props){
 ...
 const onKeyDown=(event:React.KeyboardEvent)=>{const key=event.key.toUpperCase();if(key==="ENTER"&&!event.shiftKey&&!event.ctrlKey&&!event.altKey&&!event.metaKey){const target=event.target as HTMLElement;if(target.tagName!=="TEXTAREA"&&target.tagName!=="BUTTON"){const controls=Array.from(event.currentTarget.querySelectorAll<HTMLElement>('input:not([disabled]), select:not([disabled]), [role="combobox"]:not([aria-disabled="true"]), button:not([role="tab"]):not([data-dialog-close])')).filter(control=>control.offsetParent!==null);const index=controls.indexOf(target);if(index>=0){event.preventDefault();event.stopPropagation();(controls[index+1]||confirmButtonRef.current)?.focus();return}}}const target=entries.find(entry=>entry.shortcut===key&&tabs.some(t=>t.key===entry.key));if(target){event.preventDefault();event.stopPropagation();setTab(target.key)}else if(key==="F3"){event.preventDefault();event.stopPropagation();if(!busy)onConfirm()}}
 return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent dir="rtl" onKeyDown={onKeyDown} onOpenAutoFocus={event=>{event.preventDefault();requestAnimationFrame(()=>{amountInputRef.current?.focus();amountInputRef.current?.select()})}} onCloseAutoFocus={event=>{if(onSavedClose?.())event.preventDefault()}} className="pos-payment-dialog flex max-h-[94dvh] w-[min(94vw,850px)] max-w-none flex-col gap-0 overflow-hidden p-0 text-slate-900 sm:w-[min(90vw,850px)]">
  ...
  <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-6">
   ...
   <div className="space-y-2 rounded-xl border border-blue-200 bg-white p-3 text-sm sm:p-4"><div className="flex justify-between"><span>مبلغ الفاتورة</span><strong>{money(total)} {currencyCode}</strong></div><div className="flex justify-between"><span>المبلغ المدفوع</span><strong>{money(paid)} {currencyCode}</strong></div><div className="flex justify-between border-t border-blue-500 pt-2 text-lg font-black"><span className={remaining>0.009?"text-red-600":"text-emerald-700"}>{changeDue>0.009?"المبلغ للإرجاع":"المبلغ المتبقي"}</span><span>{money(changeDue>0.009?changeDue:Math.max(0,remaining))} {currencyCode}</span></div></div>
    ...
    <div className="pos-payment-dialog-footer flex shrink-0 flex-wrap items-center justify-center gap-2 border-t bg-white p-3"><Button ref={confirmButtonRef} onClick={onConfirm} disabled={busy} className="pos-payment-confirm"><Check className="ml-1 h-4 w-4"/>موافق F3</Button><Button data-dialog-close onClick={()=>onOpenChange(false)} className="pos-payment-close"><X className="ml-1 h-4 w-4"/>إغلاق Esc</Button></div>
