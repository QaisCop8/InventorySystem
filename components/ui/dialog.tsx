...
>(({ className, children, hideCloseButton, style, ...props }, ref) => {
  ...
  return <DialogPortal>
    ...
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="universal-dialog-close absolute left-4 top-4 z-50 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/95 text-slate-900 shadow-lg ring-1 ring-slate-200 transition-opacity duration-200 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-4 w-4" />
