...
type TypedDropdownState = {
  floatLabel: boolean
}

const documentRoot = typeof document !== "undefined" ? document.documentElement : null

export default class TypedDropdown extends React.Component<TypedDropdownProps, TypedDropdownState> {
  private drpDownObj: any

  constructor(props: TypedDropdownProps) {
    super(props)
  ...
  private findOptionByValue = (value: any): DropdownOption | null => {
    ...
    return options?.find((option) => String(option?.[optionValue]) === String(value)) ?? null
  }
  ...
  private selectedOptionTemplate = (option: DropdownOption | null, props: any) => {
    const resolvedOption = option ?? this.findOptionByValue(this.props.value)
    if (!resolvedOption) {
      return <span>{props.placeholder}</span>
    }

    const optionText = this.truncatedLabel(resolvedOption)
    return (
      <div>
        <div
          style={{
  ...
  render() {
    ...
    const dropdownNode = (
      <PrimeDropdown
        ...
        id={id || "float-input"}
        title={tooltip}
        style={{ width: "100%", ...(withgroup ? { flex: "1" } : {}) }}
