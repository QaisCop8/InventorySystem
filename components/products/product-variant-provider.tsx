...
type PendingRequest = { product: ConfigurableProduct; resolve: (product: ConfigurableProduct | null) => void }
...
export function ProductVariantProvider() {
  ...
  const cancel = () => { pending.resolve(null); setPending(null); setSelection({}); setError("") }
  const confirm = () => {
    ...
    pending.resolve({ ...pending.product, product_name: summary ? `${pending.product.product_name || ""} (${summary})` : pending.product.product_name, selected_attributes: selection, attribute_summary: summary })
    setPending(null)
    setSelection({})
    setError("")
  }
  ...
  return <div className="pointer-events-auto fixed inset-0 z-[300] flex items-center justify-center bg-black/45 p-3" dir="rtl">
    <div className="flex max-h-[92dvh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5 md:grid-cols-[180px_1fr]">
        ...
        <div><h3 className="mb-5 text-lg font-bold">{pending.product.product_name}</h3><div className="space-y-5">{attributes.map((attribute) => <fieldset key={attribute.name}><legend className="mb-2 font-semibold">{attribute.name} *</legend><div className="flex flex-wrap gap-3">{attribute.values.map((value) => <label key={value} className={`flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-2 transition ${selection[attribute.name] === value ? "border-violet-600 bg-violet-50 text-violet-800 ring-1 ring-violet-600" : "hover:bg-muted"}`}><input type="radio" name={`variant-${attribute.name}`} value={value} checked={selection[attribute.name] === value} onChange={() => { setSelection((previous) => ({ ...previous, [attribute.name]: value })); setError("") }} />{value}</label>)}</div></fieldset>)}</div></div>
      </div>
      {error && <div className="mx-5 rounded-md border border-red-200 bg-red-50 p-2 text-sm font-semibold text-red-700">{error}</div>}
      <div className="flex justify-end gap-2 border-t p-4"><Button type="button" onClick={confirm}>تأكيد</Button><Button type="button" variant="outline" onClick={cancel}>إلغاء</Button></div>
    </div>
  </div>
}
