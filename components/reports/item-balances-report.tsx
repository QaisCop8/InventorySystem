...
export function ItemBalancesReport() {
  ...
  return <main dir="rtl" className="min-h-full w-full overflow-y-auto bg-[linear-gradient(145deg,rgba(248,250,252,.96),rgba(240,253,250,.75),rgba(238,242,255,.75))] p-3 sm:p-5 lg:p-7 print:bg-white print:p-0">
    <div className="mx-auto w-full max-w-[1600px] space-y-5">
      <header className="relative overflow-hidden rounded-[30px] bg-gradient-to-l from-teal-500 to-emerald-600 p-6 text-white shadow-2xl sm:p-8"><div className="relative flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-4"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/15 ring-1 ring-white/25"><FileBarChart className="h-7 w-7" /></span><div><Badge className="mb-2 border-white/20 bg-white/15 text-white">تقارير الأصناف</Badge><h1 className="text-2xl font-black sm:text-3xl">أرصدة الأصناف بتاريخ معين</h1><p className="mt-1 text-sm text-white/80">عرض أرصدة الأصناف وحركات المخزون حتى التاريخ المحدد</p></div></div><div className="flex gap-2 print:hidden"><Button variant="secondary" onClick={exportCsv} disabled={!rows.length}><Download className="ml-2 h-4 w-4" />تصدير</Button><Button className="bg-white text-slate-900 hover:bg-slate-100" onClick={() => window.print()} disabled={!rows.length}><Printer className="ml-2 h-4 w-4" />طباعة</Button></div></div></header>
